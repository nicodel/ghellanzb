...
class ghellanzb_applet:
    def __init__(self, applet, window_mode, debug):
        ...
        verb_list = [("About", self.__display_about_dialog),
                     ("Statistics", self.__display_stats_dialog)]
    ...
    def update_status(self, event=None):
        ...
        try:
            ...
            log.logger.info('self.status is %s' % self.status)
            log.logger.info('self.access is %s' % self.access)
            if self.access == 1:
                self.extract_status()
        except:
            self.status = 'Unknown'
        ...
        log.logger.info('Widget list : %s' % widget_list)
        if len(widget_list) != 0:
            for widget in widget_list:
                self.toolbar_hbox.remove(widget)
                log.logger.info('Removing widget : %s' % widget)
        ...
        if self.access != 0:
            if self.status == 'Nothing to do':
                ...
                log.logger.info('Downloading and Processing')
        else:
            icon_file = images_dir + '/ghellanzb_error_16.png'
            log.logger.info(self.status)
        ...
        log.logger.info('Adding widget %s' % image)
    ...
    def create_details_window(self):
        ...
        if len(self.hella_status['currently_processing']) != 0: 
            log.logger.debug("self.hella_status['currently_processing'] is : %s" % self.hella_status['currently_processing'])
    ...
    def get_docking_data(self, middle, w=0, h=0):
        ...
        elif orient == gnomeapplet.ORIENT_DOWN or self.window_mode:
            y += button_h
    ...
    def __display_stats_dialog(self, uicomponent=None, verb=None):
        
        print 'display stats dialog'
        o_stats = stats.get_overall()
        stats.display_dialog(o_stats)
